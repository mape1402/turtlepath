...
{
    private const string ScopeKey = "TurtlePath.Spider.Transactions.TransactionExecutionBoundary.Scope";
    private readonly ITransactionBoundaryRequestFilter requestFilter;
    private readonly TransactionBoundaryOptions options;

    /// <summary>Initializes a new transaction execution boundary.</summary>
    public TransactionExecutionBoundary(
        IOptions<TransactionBoundaryOptions> options,
    ...
    {
        if (!requestFilter.ShouldOpenTransaction(context?.RequestType))
