...
`MaxConcurrency` protects dependencies such as the database and external services. `QueueCapacity` absorbs short bursts while handlers are busy. `null` or a value below `1` leaves the corresponding limit unbounded. This controls consumer dispatch and is independent from the outbox `DispatchBatchSize` setting.
...
The template includes a test project ready for TurtlePath unit and integration tests. Use `TemplateTestHost` from `tests/Heroes.Service.Tests/Testing` so each test only adds the feature-specific maps, validators, seeds, or query profiles it needs.
...
`TurtlePath.Testing.Integration` composes TurtlePath with the testing packages used by the integrated adapters:
...
Use the delegate helpers for isolated unit tests and the adapter testing packages when the test should prove the real maps, validators, filters, messaging, or pipeline boundaries.
...
- Replace `Heroes.Service.Domain.Identifier` usages with `TurtlePath.Domain.Identifier`.
- Replace `Heroes.Service.Domain.Contracts` usages with `TurtlePath.Domain.Contracts`.
- Replace `Heroes.Service.Business.Core.Commands` usages with `TurtlePath.Commands`.
- Replace `Heroes.Service.Business.Core.Queries` usages with `TurtlePath.Queries`.
- Replace `Heroes.Service.Business.Core.Models.Requests` usages with `TurtlePath.Models.Requests`.
- Replace `Heroes.Service.Business.Core.Models.Responses` usages with `TurtlePath.Models.Responses`.
- Replace `Heroes.Service.Business.Core.Hooks` usages with `TurtlePath.Hooks`.
- Replace `Heroes.Service.Business.Core.Exceptions` usages with `TurtlePath.Exceptions` or `TurtlePath.Validation`.
...
- `Heroes.Service.Domain` keeps service-owned entities and domain code only.
- `Heroes.Service.Business` keeps service-owned commands, queries, validators, mappings, hooks, automations, and services.
- `Heroes.Service.Persistence` keeps the concrete EF Core context and service-owned EF configurations only.
...
```powershell
dotnet restore Heroes.Service.sln --verbosity minimal
dotnet build Heroes.Service.sln --configuration Release --no-restore --verbosity minimal
dotnet test Heroes.Service.sln --configuration Release --no-build --verbosity minimal
```
