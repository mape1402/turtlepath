...
El template incluye un proyecto de pruebas listo para unit tests e integration tests de TurtlePath. Usa `TemplateTestHost` desde `tests/Heroes.Service.Tests/Testing` para que cada prueba solo agregue los maps, validators, seeds o query profiles específicos del feature.
...
`TurtlePath.Testing.Integration` compone TurtlePath con los paquetes de testing usados por los adapters integrados:
...
Usa los helpers con delegates para unit tests aislados y los paquetes de testing de adapters cuando la prueba debe validar los maps, validators, filtros, messaging o boundaries reales.
...
- Reemplaza usos de `Heroes.Service.Domain.Identifier` con `TurtlePath.Domain.Identifier`.
- Reemplaza usos de `Heroes.Service.Domain.Contracts` con `TurtlePath.Domain.Contracts`.
- Reemplaza usos de `Heroes.Service.Business.Core.Commands` con `TurtlePath.Commands`.
- Reemplaza usos de `Heroes.Service.Business.Core.Queries` con `TurtlePath.Queries`.
- Reemplaza usos de `Heroes.Service.Business.Core.Models.Requests` con `TurtlePath.Models.Requests`.
- Reemplaza usos de `Heroes.Service.Business.Core.Models.Responses` con `TurtlePath.Models.Responses`.
- Reemplaza usos de `Heroes.Service.Business.Core.Hooks` con `TurtlePath.Hooks`.
- Reemplaza usos de `Heroes.Service.Business.Core.Exceptions` con `TurtlePath.Exceptions` o `TurtlePath.Validation`.
...
- `Heroes.Service.Domain` conserva solo entidades y código de dominio propio del servicio.
- `Heroes.Service.Business` conserva commands, queries, validators, mappings, hooks, automations y services propios del servicio.
- `Heroes.Service.Persistence` conserva el contexto EF Core concreto y configuraciones EF propias del servicio.
